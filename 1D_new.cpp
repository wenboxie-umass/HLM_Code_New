...
void update(T &clock_time_in_step, P &time_array, Q &energy_array, const int N, const double small_tau, const int ratio, const int Step, trng::uniform01_dist<> &u, trng::yarn2 &r, int &count, const int level) {
...
//	array<custom_priority_queue<interaction>, ratio + 1> a;
//	
//	interaction item;
//	
//	item.time = 0.001;
...
//	a[0].print_queue();
//	
//	move_interaction(a, item, small_tau, ratio, 0, 0.002, 0);
//	
//	a[0].print_queue();
...
//				cout<<"Current Level: "<<j<<" start from: "<<double(j) * small_tau<<endl;
//				clock_time_in_step[j].print_queue();
//			}
//		}
		...
		for(int j = 0 ; j < ratio; j ++) {
			if(!clock_time_in_step[j].empty()) {
				update(clock_time_in_step, time_array, energy_array, N, small_tau, ratio, i, u, r, count, j);
				//break;
			}
		}
		...
		bool found = false;
		string ret = (clock_time_in_step[ratio].size() == N + 1) ? "Yes, all clocks are in ratio level" : "No, Check Algorithm";
		cout<<ret<<endl;
		bool res = clock_time_in_step[ratio].check_value((i + 1) * small_tau * ratio);
		//cout<<"Properly Updated? "<<res<<endl;
//		if(!res.compare("Clear")) {
//			//cout<<"Good!"<<endl;
//			cout<<"Uh Oh, Check Algorithm"<<endl;
//			exit(0);
//		}
		if(res) {
			cout<<"Properly Updated? Yes"<<endl;
		}
		else {
			cout<<"Uh Oh, Check Algorithm"<<endl;
			exit(0);
